(function(){ 'use strict';
    ...
    const createCamera = window.createCamera;
    const createLights = window.createLights;
    const createScene = window.createScene;
    const createRenderer = window.createRenderer;
    const createPlot = window.createPlot;
    const disposeObject = window.disposeObject;
    const UndoManager = window.UndoManager;
    const AxisIndicator = window.AxisIndicator;
    const Loop = window.Loop;
    const Rulers = window.Rulers;
    const ControlsPanel = window.ControlsPanel;
    ...
    class World {
        ...
        _createMeshFromDefinition(itemData) {
            if (!itemData || typeof itemData !== 'object' || !itemData.name || typeof itemData.width !== 'number' || typeof itemData.height !== 'number') {
                throw new Error('_createMeshFromDefinition: invalid itemData. Expected object with name, width, height.');
            }
        ...
        enterPlantInPlanterSelectionMode(planter) {
            if (!planter?.userData?.isPlanter) {
                throw new Error('enterPlantInPlanterSelectionMode: object is not a planter. Expected userData.isPlanter === true.');
            }
            this.deselectObject();
            ...
            this.infoPanel.show(`Add Plant to ${planter.name}`, 'Click a plant in the right panel, then click on the planter soil to place. Press Esc to cancel.', null);
        }

        activatePlantInPlanterPlacement(plantDef) {
            if (!this.activePlanter) {
                throw new Error('activatePlantInPlanterPlacement: activePlanter is null. Enter planter mode first.');
            }
            if (!plantDef || typeof plantDef !== 'object' || !plantDef.name || typeof plantDef.width !== 'number' || typeof plantDef.height !== 'number') {
                throw new Error('activatePlantInPlanterPlacement: invalid plantDef. Expected object with name, width, height.');
            }
            this.activePlanterHistory = [];
            this._savePlanterStateToHistory();
        ...
        addPlanterActionButtons(pane) {
            ...
            actionsFolder.addButton({ title: 'â†©ï¸ Undo Last Action' }).on('click', () => this.undoLastPlanterOperation());
            actionsFolder.addButton({ title: 'ðŸ—‘ï¸ Remove All Plants' }).on('click', () => this.removeAllPlantsInPlanter());
            actionsFolder.addButton({ title: 'ðŸŒ± Choose Different Plant' }).on('click', () => this.backToPlantSelection());
        }

        addPopulateOptions(pane) {
            if (!this.activePlanter?.userData?.dimensions) {
                throw new Error('addPopulateOptions: activePlanter has no dimensions. Planter may be corrupted.');
            }
        ...
        backToPlantSelection() {
            if (!this.activePlanter) {
                console.error('backToPlantSelection: activePlanter is null. Exiting planter mode.');
                this.exitPlantInPlanterSelectionMode();
                return;
            }
            if (this.plantPlacementPreview) {
            ...
            this.mode = 'PLANT_IN_PLANTER_SELECT_PLANT';
            this.showPlantSelectionPanel();
            this.infoPanel.show(`Add Plant to ${this.activePlanter.name}`, 'Click a plant in the right panel, then click on the planter soil to place. Press Esc to cancel.', null);
        }
        ...
        showPlantSelectionPanel() {
            ...
            for (const category in PLANTS) {
                const folder = pane.addFolder({ title: category.charAt(0).toUpperCase() + category.slice(1), expanded: true });
        ...
        placePlantInPlanter(plantDef, localPosition, rotationY) {
            if (!this.activePlanter || !plantDef) {
                throw new Error('placePlantInPlanter: activePlanter or plantDef is missing. Cannot place plant.');
            }
            this._savePlanterStateToHistory();
